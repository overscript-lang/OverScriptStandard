...
using System.IO;
using System.Linq;
using static OverScriptStandard.Literals;
...
namespace OverScriptStandard
{
    public partial class ScriptClass
    {
        ...
        public const char CodeLocationChar = (char)127;
        ...
        public const char LM_0 = (char)15;
        public const char LM_1 = (char)16;
            ...
            while (i >= 0)
            {
                ...
                if (c != '_' && !Char.IsLetterOrDigit(c))
                {
                    ...
                    while (i3 > i && i3 < i2)
                    {
                        ...
                        i3 = code.EIndexOf(" in", i3 + 1);
                    }
                }
                i = code.EIndexOf("foreach(", i + 1);
            }
        }
        ...
        static void ReplaceDoubleDot(ref string code)
        {
            ...
            while (i >= 0)
            {
                ...
                i = code.IndexOf("..", i + 1, StringComparison.Ordinal);
            }
        }
                ...
                try
                {
                    ...
                    else if (s == "}")
                    {
                        ...
                        else if (currentBlockType == UnitType.Do)
                        {
                            ...
                            if (!lines[i].EStartsWith("while(")) { i--; throw new ScriptLoadingException("'do' block must end with 'while' statement."); }
                    ...
                    else if (s.EStartsWith("goto "))
                    {
                        units[i].Type = UnitType.GoTo;
                        s = s.Substring(5);

                    }
                    else if (s.EStartsWith("if(")) { units[i].Type = UnitType.If; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("for(")) { units[i].Type = UnitType.For; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("foreach(")) { units[i].Type = UnitType.ForEach; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("while(")) { units[i].Type = UnitType.While; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("elseif(")) { units[i].Type = UnitType.ElseIf; types.Push(units[i].Type); s = GetUnitStr(s); units[i - 1].Type = UnitType.EndIfTrueBlock; }
                    else if (s.EStartsWith("switch(")) { units[i].Type = UnitType.Switch; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("return(")) { units[i].Type = UnitType.Return; s = GetUnitStr(s); }
                    else if (s.EStartsWith("return ")) { units[i].Type = UnitType.Return; s = s.Substring(7); }
                    else if (s.EStartsWith("throw(")) { units[i].Type = UnitType.Throw; s = GetUnitStr(s); s = string.Join(ServiceChar, SmartSplit(s)); }
                    else if (s.EStartsWith("throw ")) { units[i].Type = UnitType.Throw; s = s.Substring(6); }
                    else if (s.EStartsWith("apply ")) { units[i].Type = UnitType.Apply; s = s.Substring(6); }
                    else if (s.EStartsWith("apply$ ")) { units[i].Type = UnitType.Reapply; s = s.Substring(7); }
                    else if (s.EStartsWith("apply(")) { units[i].Type = UnitType.Apply; s = GetUnitStr(s); }
                    else if (s.EStartsWith("apply$(")) { units[i].Type = UnitType.Reapply; s = GetUnitStr(s); }
                    ...
                    else if (s.EStartsWith("catch(")) { units[i].Type = UnitType.Catch; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EStartsWith("catch$(")) { units[i].Type = UnitType.CatchByName; types.Push(units[i].Type); s = GetUnitStr(s); }
                    else if (s.EndsWith(":"))
                    {
                        if (s.EStartsWith("case "))
                        {
        ...
        private static void ItemizeMultiDeclaring(ref string code)
        {
            ...
            while (i >= 0)
            {
                if (!InsideBlock(code, i, "(", ")"))
                {
                    ...
                    i = code.LastIndexOf(' ', i2 - 1);
                }
                else { i = code.LastIndexOf(' ', i - 1); }
                    ...
                    else if (st == StatementKind.Do)
                    {
                        ...
                        if (code[u3] != '{')
                        {
                            ...
                            if (CheckBlockStatementStarts(code, u3, out bs))
                            {
                                ...
                                FindStatementEnd(code, u, ref u2, st);
                                if (bs == StatementKind.Do) u2 = code.IndexOf(';', u2 + 1);
                            }
                            else
                                u2 = code.IndexOf(';', u3);
        ...
        static int FindNextBreakBrace(string code, int u, StatementKind st)
        {
            ...
            do
            {
                ok = true;
                s2 = s.Substring(p + 1);
                if (s2.EStartsWith("catch{") || s2.EStartsWith("catch(") || s2.EStartsWith("finally{")) ok = false;
                else if (st != StatementKind.Do && IsEndDo(s2)) ok = false;
                else if (s2.EStartsWith("elseif(") || s2.EStartsWith("else{"))
                {
        ...
        {
            if (!s.EStartsWith("while(")) return false;
        ...
        static int FindLastStatementToAddBraces(string code, int start, out StatementKind st)
        {
            ...
            {
                int i = code.ELastIndexOf(AddBracesStatements[n], start);
                if (i >= 0 && (i > pos || pos < 0))
                {
        ...
        static StatementKind GetStatementKind(string code, int start)
        {
            ...
            for (int n = 0; n < AddBracesStatements.Length; n++)
            {
                ...
                if (code.EIndexOf(s, start) == start) return (StatementKind)n;
            }
        ...
        static bool CheckBlockStatementStarts(string code, int start, out StatementKind bs)
        {
            ...
            for (int n = 0; n < BlockStatements.Length; n++)
            {
                ...
                if (code.EIndexOf(s, start) == start) { bs = (StatementKind)n; return true; }
            }
        ...
        private static bool InsideBlock(string text, int pos, string char1, string char2, int startPos = 0)
        {
            ...
            h = text.EIndexOf(char1, startPos);
            while (h >= 0 && h < pos) { k1++; h = text.EIndexOf(char1, h + 1); }

            h = text.EIndexOf(char2, startPos);
        ...
        static void GetFuncPrefix(ref string fnName, out bool orBasic, out bool orLocal, out bool basic)
        {
            ...
            if (orBasic = fnName.EStartsWith(OrBasicFunctionPrefix)) fnName = fnName.Remove(0, OrBasicFunctionPrefix.Length);
            else if (orLocal = fnName.EStartsWith(OrLocalFunctionPrefix)) fnName = fnName.Remove(0, OrLocalFunctionPrefix.Length);
        ...
        private ExpressionType DirectGetExpressionType(string strToEval, Dictionary<string, VarType> varTypes, ref bool noCache, EvalUnit parent = null, OperandPosition operandPos = default, string otherPart = null)
        {
            ...
            if (opPos < 0 && strToEval.EStartsWith("(") && strToEval.EndsWith(")"))
            {
                int bp = strToEval.EIndexOf(")(");
                if (bp >= 0)
                {
            ...
            if (opPos >= 0)
            {
                ...
                if (isCasting)
                {
                    bool byRef = part1.EStartsWith("ref ");
            ...
            else if (strToEval.EStartsWith("new ") && SmartCharPos(strToEval, '.') <= strToEval.IndexOfAny(Brackets))
            {
            ...
            find:
                ...
                if (!basic)
                {
                    ...
                    if (!local)
                    {
                        bool? onlyPublic = isThis ? (Nullable<bool>)null : true;
            ...
            {
                VarType t;
                bool byRef = strToEval.EStartsWith("ref ");
                if (byRef) strToEval = strToEval.Remove(0, 4);
                strToEval = RemModifs(strToEval);
                int spacePos = strToEval.IndexOf(' ');

                if (spacePos >= 0 && !strToEval.EStartsWith("new ") && !strToEval.StartsWith('(') && !InBrackets(strToEval, spacePos))
                {
        ...
        private static int FindOperator(string str, ref OperationKind foundOperator)
        {
            ...
            for (int i = c; i >= 0; i--)
            {
                ...
                {
                    if (noRev)
                        pos = str.IndexOf(op, ++pos, StringComparison.Ordinal);
                    else
                        pos = str.LastIndexOf(op, --pos, StringComparison.Ordinal);
        ...
        {
            if (s.EStartsWith("public ")) s = s.Remove(0, 7);
        ...
        private EvalUnit GetEvalUnit<T1>(string strToEval, Dictionary<string, VarType> varTypes, ref bool noCache, EvalUnit parent = null, OperandPosition operandPos = default, string otherPart = null)
        {
            ...
            if (s >= 0)
            {
                startsWith_new = strToEval.EStartsWith("new ");
                startsWith_ref = strToEval.EStartsWith("ref ");
            ...
            if (result.Kind == EvalUnitKind.Variable || result.Kind == EvalUnitKind.Function || result.Kind == EvalUnitKind.New || result.Kind == EvalUnitKind.ArrayItem)
            {
                ...
                if (result.Kind == EvalUnitKind.Function || result.Kind == EvalUnitKind.ArrayItem || result.Kind == EvalUnitKind.New)
                {
                    ...
                    if (result.Kind == EvalUnitKind.New && opr.EStartsWith("{")) opr = "[]" + opr;
                ...
                if (result.Kind == EvalUnitKind.Function)
                {
                    ...
                    find:
                        ...
                        else
                        {
                            ...
                            var ff = sc.GetFunc<T1>(fnName, args, true, isThis ? (Nullable<bool>)null : true, !isStatic ? (Nullable<bool>)null : (Nullable<bool>)isStatic);
                ...
                else if (result.Kind == EvalUnitKind.New && (InnerType.ID == TypeID.Custom || InnerType.ID == TypeID.CustomArray))
                {
                    ...
                    if (InnerType.ID == TypeID.Custom)
                    {
                        ...
                        bool? pub = result.Type.CType.Class.Is(this) ? (Nullable<bool>)null : true;
        ...
        private ScriptFunction FindFuncInThisOrOuters(string name, EvalUnit[] args, bool onlyStatic = false)
        {
            ...
            bool? isStatic = onlyStatic ? true : (Nullable<bool>)null;
        ...
        public static FuncToCall<T> GetFuncToCall<T>(ScriptFunction f)
        {
            if (f == null) return null;
            return (EvalUnit[] fnArgs, int baseScope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => ExecuteFunction<T>(f, fnArgs, baseScope, srcInst, inst, cstack, csrc);
        }
        ...
        private static int SmartCharPos(string str, char c = '.', int start = 0)
        {
            ...
            while (i >= 0 && InBrackets(str, i)) i = str.IndexOf(c, i + 1);
            return i;
        }
        private static int SmartCharPosRev(string str, char c = '.', int start = -1)
        {
            ...
            while (i >= 0 && InBrackets(str, i)) i = str.LastIndexOf(c, i - 1);
            return i;
        }
