...
using System.Web;
using static OverScriptStandard.Executor;
using static OverScriptStandard.ScriptClass;
...
namespace OverScriptStandard
{
    ...
    public partial class BasicFunctions
    {
        ...
        public OverloadVariant GetFunc(string name, ref EvalUnit[] args, bool noException, ScriptClass cc)
        {
            ...
            if (name.EStartsWith("op_") && name.EndsWith("Casting"))
            {
        ...
        private static double Log2_double(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Math.Log(fnArgs[0].EvalDouble(scope, inst, cstack), 2);
        ...
        {
            string dest = GetFullPath(fnArgs[1].EvalString(scope, inst, cstack), cstack, csrc);
            if (fnArgs[2].EvalBool(scope, inst, cstack) && File.Exists(dest)) File.Delete(dest);
            File.Move(GetFullPath(fnArgs[0].EvalString(scope, inst, cstack), cstack, csrc), dest);
            return null;
        }
        ...
        private static object FromJson(string json, TypeArg typeArg, bool withPrivate, int scope, ClassInstance inst, CallStack cstack)
        {
            ...
            if (typeArg.Id == TypeID.None)
                return JsonConvert.DeserializeObject(json, typeArg.Type);

            var jsElement = JToken.Parse(json);

            return DeserializeJson(in jsElement, typeArg.Id, typeArg.CType, withPrivate, scope, inst, cstack);
        }
        private static object DeserializeJson(in JToken je, TypeID type, CustomType ctype, bool withPrivate, int scope, ClassInstance inst, CallStack cstack)
        {
            ...
            if (isCustomArray || type == TypeID.Custom)
            {
                ...
                if (isCustomArray)
                {
                    ...
                    return isCustomArray ? (object)new CustomObject(inst.Exec, arr, ctype.Class, true) : arr;
                ...
                for (int i = 0; i < vars.Count; i++)
                {
                    ...
                    if (el!=null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (v.Type.TypeHint != null)
                                {
                                    object val = el.ToObject(v.Type.TypeHint); 
                                    ScriptVars<object>.Set(exec, varId, newScope, ref val);

                                } else { 
                                    switch (v.Type.ID)
                                    {
                                        case TypeID.Int: { int val = el.Value<int>(); ScriptVars<int>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Long: { long val = el.Value<long>(); ScriptVars<long>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Byte: { byte val = el.Value<byte>(); ScriptVars<byte>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Short: { short val = el.Value<short>(); ScriptVars<short>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Float: { float val = el.Value<float>(); ScriptVars<float>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Double: { double val = el.Value<double>(); ScriptVars<double>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Decimal: { decimal val = el.Value<decimal>(); ScriptVars<decimal>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Bool: { bool val = el.Value<bool>(); ScriptVars<bool>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Date: { DateTime val = el.Value<DateTime>(); ScriptVars<DateTime>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.String: { string val = el.Value<string>(); ScriptVars<string>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Char: { char val = el.Value<char>(); ScriptVars<char>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.Object: { object val = el.Value<object>(); ScriptVars<object>.Set(exec, varId, newScope, ref val); break; }
                            ...
                            {
                                var jarr = el as JArray;
                                if (v.Type.TypeHint != null)
                                {
                                    Type itemType = v.Type.TypeHint.GetElementType();
                                    object[] val = jarr.Select(x => x.ToObject(itemType)).ToArray();
                                    ScriptVars<object[]>.Set(exec, varId, newScope, ref val);
                                ...
                                else
                                {
                                    ...
                                    switch (v.Type.ID)
                                    {
                                        case TypeID.IntArray: { int[] val = jarr.Select(x => x.Value<int>()).ToArray(); ScriptVars<int[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.LongArray: { long[] val = jarr.Select(x => x.Value<long>()).ToArray(); ScriptVars<long[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.ByteArray: { byte[] val = jarr != null ? jarr.Select(x => x.Value<byte>()).ToArray() : Convert.FromBase64String(el.Value<string>()); ScriptVars<byte[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.ShortArray: { short[] val = jarr.Select(x => x.Value<short>()).ToArray(); ScriptVars<short[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.FloatArray: { float[] val = jarr.Select(x => x.Value<float>()).ToArray(); ScriptVars<float[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.DoubleArray: { double[] val = jarr.Select(x => x.Value<double>()).ToArray(); ScriptVars<double[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.DecimalArray: { decimal[] val = jarr.Select(x => x.Value<decimal>()).ToArray(); ScriptVars<decimal[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.BoolArray: { bool[] val = jarr.Select(x => x.Value<bool>()).ToArray(); ScriptVars<bool[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.DateArray: { DateTime[] val = jarr.Select(x => x.Value<DateTime>()).ToArray(); ScriptVars<DateTime[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.StringArray: { string[] val = jarr.Select(x => x.Value<string>()).ToArray(); ScriptVars<string[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.CharArray: { char[] val = jarr.Select(x => x.Value<char>()).ToArray(); ScriptVars<char[]>.Set(exec, varId, newScope, ref val); break; }
                                        case TypeID.ObjectArray: { object[] val = jarr.Select(x => x.Value<object>()).ToArray(); ScriptVars<object[]>.Set(exec, varId, newScope, ref val); break; }
                                        default: throw new ScriptExecutionException($"Wrong type '{v.Type.ID}'.");
                                    }
                                }
                            }
            ...
            switch (type)
            {
                case TypeID.Int: return je.Value<int>();
                case TypeID.Long: return je.Value<long>();
                case TypeID.Byte: return je.Value<byte>();
                case TypeID.Short: return je.Value<short>();
                case TypeID.Float: return je.Value<float>();
                case TypeID.Double: return je.Value<double>();
                case TypeID.Decimal: return je.Value<decimal>();
                case TypeID.Date: return je.Value<DateTime>();
                case TypeID.Bool: return je.Value<bool>();
                case TypeID.String: return je.Value<string>();
                case TypeID.Char: return je.Value<char>();
                case TypeID.Object: return je.Value<object>();

                case TypeID.IntArray: return ((JArray)je).Select(x => x.Value<int>()).ToArray<int>();
                case TypeID.LongArray: return ((JArray)je).Select(x => x.Value<long>()).ToArray<long>();
                case TypeID.ByteArray: return je is JArray jarr ? jarr.Select(x => x.Value<byte>()).ToArray<byte>() : Convert.FromBase64String(je.Value<string>());
                case TypeID.ShortArray: return ((JArray)je).Select(x => x.Value<short>()).ToArray<short>();
                case TypeID.FloatArray: return ((JArray)je).Select(x => x.Value<float>()).ToArray<float>();
                case TypeID.DoubleArray: return ((JArray)je).Select(x => x.Value<double>()).ToArray<double>();
                case TypeID.DecimalArray: return ((JArray)je).Select(x => x.Value<decimal>()).ToArray<decimal>();
                case TypeID.DateArray: return ((JArray)je).Select(x => x.Value<DateTime>()).ToArray<DateTime>();
                case TypeID.BoolArray: return ((JArray)je).Select(x => x.Value<bool>()).ToArray<bool>();
                case TypeID.StringArray: return ((JArray)je).Select(x => x.Value<string>()).ToArray<string>();
                case TypeID.CharArray: return ((JArray)je).Select(x => x.Value<char>()).ToArray<char>();
                case TypeID.ObjectArray: return ((JArray)je).Select(x => x.Value<object>()).ToArray<object>();
        ...
        private static string Fetch_string(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack));
        private static string Fetch_string_string(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack), fnArgs[1].EvalString(scope, inst, cstack));
        private static string Fetch_string_string_string(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack), fnArgs[1].EvalString(scope, inst, cstack), fnArgs[2].EvalString(scope, inst, cstack));
        private static string Fetch_string_string_string_int(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack), fnArgs[1].EvalString(scope, inst, cstack), fnArgs[2].EvalString(scope, inst, cstack), fnArgs[3].EvalInt(scope, inst, cstack));

        private static string Fetch_string_string_string_int_string(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack), fnArgs[1].EvalString(scope, inst, cstack), fnArgs[2].EvalString(scope, inst, cstack), fnArgs[3].EvalInt(scope, inst, cstack), StrToHttpClientHandler(fnArgs[4].EvalString(scope, inst, cstack)));
        private static string Fetch_string_string_string_int_object(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc) => Fetch(fnArgs[0].EvalString(scope, inst, cstack), fnArgs[1].EvalString(scope, inst, cstack), fnArgs[2].EvalString(scope, inst, cstack), fnArgs[3].EvalInt(scope, inst, cstack), (HttpClientHandler)fnArgs[4].EvalObject(scope, inst, cstack));
        ...
        private static string Fetch_string_string_string_int_object_string_string_string(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc)
        {
            ...
            return Fetch(uri, headers, encodingName, timeout, handler, postData, postDataEncoding, postDataType);
        }
        ...
        private static string Fetch(string uri, string headers = "", string encName = "", int timeout = 0, HttpClientHandler handler = null, string postData = "", string postDataEncoding = "UTF-8", string postDataType = "application/x-www-form-urlencoded")
        {
            string data = "";
            using (var client = handler != null ? new HttpClient(handler) : new HttpClient())
            {
                ...
                if (postData.Length > 0)
                {
                    ...
                    response = client.PostAsync(uri, postContent).Result;
                }
                else response = client.GetAsync(uri).Result;
                ...
                Encoding enc = string.IsNullOrEmpty(encName) ? Encoding.ASCII : GetEncodingOrDefault(encName);
            ...
            Encoding GetEncodingOrDefault(string name)
            {
                try { return Encoding.GetEncoding(encName); }
        ...
        private static (ScriptFunction fn, ClassInstance inst, EvalUnit[] args) DynGetFunc(EvalUnit[] fnArgs, int scope, ClassInstance inst, CallStack cstack)
        {
            ...
            var fn = onInst.Class.GetFunc(fnName, args, false, null, isStatic ? true : (Nullable<bool>)null);
            if (fn == null)
        ...
        private static object Enumerator_object_object_object_object(EvalUnit[] fnArgs, int scope, ClassInstance inst, CallStack cstack, EvalUnit csrc, bool byFnRef)
        {
            ...
            Action dispose = null;
            if(disposeFnRef != null) dispose=() => CallFunc(disposeFnRef.Func, disposeFnRef.Inst, null, scope, inst, cstack, csrc);
        ...
        private static object Enumerator_object(EvalUnit[] fnArgs, int scope, ClassInstance srcInst, ClassInstance inst, CallStack cstack, EvalUnit csrc)
        {
            ...
            Action dispose = null;
            if(c.DisposeFunc != null) dispose=() => CallFunc(c.DisposeFunc, ci, null, scope, inst, cstack, csrc);
            return new Enumerator(moveNext, current, reset, dispose);
        }
        ...
        private static string IniGet(string str, int startPos, int stopPos)
        {
            if (startPos >= 0)
            {
                ...
                if (v.EStartsWith("\"") && v.EndsWith("\"") && v.Length > 1)
                {
