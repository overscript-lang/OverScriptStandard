    ...
    public partial class ScriptClass
    {
        ...
        private void GetSubClasses(ref string code)
        {
            ...
            while (i >= 0)
            {
                ...
                if (pc == ' ' || pc == CodeLocationChar || pc == UnitSeparator[0])
                {
                    ...
                    className = RemLocationMarks(className).Trim();
        ...
        private void GetClassFuncCodes(ref string code)
        {
            ...
            while (i >= 0)
            {
                ...
                if (!funcHead.EStartsWith("if(") && !funcHead.EStartsWith("elseif(") && !funcHead.EStartsWith("for(") && !funcHead.EStartsWith("foreach(") && !funcHead.EStartsWith("while(") && !funcHead.EStartsWith("switch(") && !funcHead.EStartsWith("catch(") && !funcHead.EStartsWith("catch$(") && !funcHead.EStartsWith("lock(") && !funcHead.EStartsWith("using("))
                {
                    i3 = FindClosing(code, i, "{", "}");
        ...
        public class VarsToClear
        {
            ...
            {
                UsedTypeCount = 0;
                if(StringUsed = (UsedTypes & TypeFlag.String) != 0) UsedTypeCount++;
                if (ObjectUsed = (UsedTypes & TypeFlag.Object) != 0) UsedTypeCount++;
                if (CustomUsed = (UsedTypes & TypeFlag.Custom) != 0) UsedTypeCount++;
                if (IntArrayUsed = (UsedTypes & TypeFlag.IntArray) != 0) UsedTypeCount++;
                if (StringArrayUsed = (UsedTypes & TypeFlag.StringArray) != 0) UsedTypeCount++;
                if (ByteArrayUsed = (UsedTypes & TypeFlag.ByteArray) != 0) UsedTypeCount++;
                if (LongArrayUsed = (UsedTypes & TypeFlag.LongArray) != 0) UsedTypeCount++;
                if (DoubleArrayUsed = (UsedTypes & TypeFlag.DoubleArray) != 0) UsedTypeCount++;
                if (FloatArrayUsed = (UsedTypes & TypeFlag.FloatArray) != 0) UsedTypeCount++;
                if (BoolArrayUsed = (UsedTypes & TypeFlag.BoolArray) != 0) UsedTypeCount++;
                if (DecimalArrayUsed = (UsedTypes & TypeFlag.DecimalArray) != 0) UsedTypeCount++;
                if (CharArrayUsed = (UsedTypes & TypeFlag.CharArray) != 0) UsedTypeCount++;
                if (ShortArrayUsed = (UsedTypes & TypeFlag.ShortArray) != 0) UsedTypeCount++;
                if (DateArrayUsed = (UsedTypes & TypeFlag.DateArray) != 0) UsedTypeCount++;
                if (ObjectArrayUsed = (UsedTypes & TypeFlag.ObjectArray) != 0) UsedTypeCount++;
                ...
                try
                {
                    ...
                    if (p.Length > 1 || p[0].Length > 0)
                    {
                        ...
                        for (int n = 0; n < p.Length; n++)
                        {
                            ...
                            string prm = p[n];
                            byRef = prm.EStartsWith("ref ");
                            if (byRef) { p[n] = prm = prm.Substring(4); byRef = true; RefCount++; }
                            else if (prm.EStartsWith("params ")) { p[n] = prm = prm.Substring(7); HasParams = true; ParamsIndex = n; }
